	...
	methods: {
		...
		scrollByIndex(tabCur, needTransition = true) {
			...
			let itemWidth = item.width || 0,
				itemLeft = (item.left - this.data.boxleft) || 0;
		...
		init() {
			...
			this.setData({
				windowWidth: this.data.boxw > 0 ? this.data.boxw : windowWidth
			});
