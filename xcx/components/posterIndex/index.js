...
const handle = {
    ...
    _drawSingleText({ x, y, fontSize, color, baseLine, textAlign = 'left', text, opacity = 1, textDecoration = 'none',
        ...
        textSplit.forEach((pitem, pindex) => {
            textWidth = this.toRpx(this.ctx.measureText(pitem).width);

            if (textWidth > width) {
                // 文本宽度 大于 渲染宽度
                let fillText = '';
                let line = 1;
                for (let i = 0; i <= pitem.length - 1; i++) {  // 将文字转为数组，一行文字一个元素
                    fillText = fillText + pitem[i];
