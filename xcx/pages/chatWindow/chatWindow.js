...
Page({
  ...
  //容器高度
  getRef() {
    _this.createSelectorQuery().selectAll(".ref").boundingClientRect((res) => {
      // console.log(res)
      let boxH = 0;
      ...
      _this.setData({
        boxH: appg.getSystemInfo.screenHeight - boxH
      });
    }).exec();
  },
  ...
  minuserChange() {
    ...
    http.userArchiveDetail(opts.userArchiveId).then(res => {
      ...
      nvabarData.title = res.name;
      nvabarData.birth = res.birth;
    ...
    http.userInspectReportPage(data).then(res => {
      if (v) {
        ...
        if (res.total > 0 && archive.age <=18){
          ...
          // console.log(speed[opts.userArchiveId],'=============1')
  ...
  getVisionReport(index) {
    ...
    // console.log('getVisionReport=====================')
    let {vision, archive, d} = _this.data;
    ...
    util.getVision(objL, function(resultLeft) {
      ...
      util.getSpeed(speedobj, archive.id, function(speedLeft) {
        ...
        _this.setData({ speed: storageSpeed })
      })
