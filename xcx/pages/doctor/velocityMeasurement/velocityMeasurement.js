...
Page({
  ...
  onLoad(options) {
    opts = options;
    _this = this;
    let {datas, datas1} = _this.data;
    datas.userArchiveId = opts.userArchiveId;
    datas1.userArchiveId = opts.userArchiveId
    _this.setData({
      datas,
      datas1,
  ...
  minuserChange() {
    ...
    if (storageSpeed[opts.userArchiveId]) {
    //   console.log(storageSpeed[opts.userArchiveId],'==============')
      _this.setData({
        speed: storageSpeed[opts.userArchiveId]
      })
      _this.getReport();
    }
    ...
    _this.getDataIntervene(_this.data.datas1)
    }).catch(res => {
      app.toast({
  ...
  getDataIntervene(data, v) {
    http.userIntervenePage(data).then(res => {
      if (v) {
        let {d1, datas1} = _this.data;
        d1.pageNum = res.pageNum;
        d1.list = d1.list.concat(res.list);

        datas1.pageNum = res.pageNum;
        _this.setData({
          d1,
          datas1
        });
   ...
   */
  onShow() {
      console.log('onShow',this.data.isRefresh)
  ...
  getReport(cb) {
    http.userInspectReportPage({
        ...
        _this.setData({ reportList: res.list});
...
getHistory(archive, report, reportList) {
  ...
  getVisionReport(index) {
    ...
    let {vision, archive, reportList} = _this.data;
    ...
    // 区间测速
    // history_farsight_records 历史散瞳验光数据的列表
    let historyALRecordsLeftSpeed = [], historyALRecordsRightSpeed = [];
    ...
    util.getVision(objL, function(resultLeft) {
      ...
      util.getSpeed(speedobj, archive.id, function(speedLeft) {
        ...
        if (storageSpeed[archive.id].l && storageSpeed[archive.id].r) {
            _this.setData({ loading: false })
        }

        _this.setData({ speed: storageSpeed[archive.id] })
      })
   ...
   */
  onReachBottom() {
    let { datas, d, datas1, d1, tabIndex } = _this.data;
    if (tabIndex == 0)  {
      ...
      if (datas1.pageNum >= d1.pages) {
        return;
      }
      datas1.pageNum = datas1.pageNum + 1;
      _this.getDataIntervene(datas1, 'add');
    }
