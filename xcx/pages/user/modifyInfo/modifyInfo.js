...
Page({
  ...
  _getAddress: function (e) {
    ...
    user.selectRegionId = v.selectRegionId.join('-');
    user.selectRegion = v.selectRegion.join('-');
  ...
  saveInfo() {
    ...
    http.userInfoPut(user).then(res => {
      app.toast({
        title: "保存成功",
        icon: 'success',
        time: 1000,
